...
/*[Arima_5830][bozhi_lin] implement diag send sync command for factory 20160223 begin*/
#if 1
#include <linux/delay.h>
#endif
/*[Arima_5830][bozhi_lin] 20160223 end*/
...
/*[Arima_5830][bozhi_lin] implement diag send sync command for factory 20160223 begin*/
#if 1
void *diag_buf_copy = NULL;
unsigned int diag_payload_size = 0;
static bool diag_done = false;
static int wait_retry = 500; //total wait 5 seconds
#endif
/*[Arima_5830][bozhi_lin] 20160223 end*/
...
/*[Arima_5830][bozhi_lin] implement diag send sync command for factory 20160223 begin*/
#if 1
	case DIAG_IOCTL_DIAG_SEND:
		{
			int retry = wait_retry;
			diag_done = false;
			//pr_info("[B] %s(%d): diag_done=%d", __func__, __LINE__, diag_done);

			if (copy_from_user(&diag_payload_size, (void *)ioarg, 4)) 
			{
				pr_info("[B] %s(%d): error copy_from_user diag_payload_size", __func__, __LINE__);
				return -EFAULT;
			}

			diag_buf_copy = diagmem_alloc(driver, diag_payload_size, POOL_TYPE_COPY);
			if (!diag_buf_copy) 
			{
				pr_info("[B] %s(%d): error diagmem_alloc", __func__, __LINE__);
				return -ENOMEM;
			}

			if (copy_from_user(diag_buf_copy, (void *)(ioarg + 4), diag_payload_size)) {
				pr_info("[B] %s(%d): error copy_from_user diag_buf_copy", __func__, __LINE__);
				return -EFAULT;
			}
		
#if 0
			printk(KERN_DEBUG "1. data is -->\n");
			for (i = 0; i < diag_payload_size; i++)
				printk(KERN_DEBUG "1.\t %x \t", *(((unsigned char *)diag_buf_copy)+i));
#endif

			diag_process_nohdlc(diag_buf_copy, diag_payload_size);
			//msleep(20);
			...
			while(retry)
			{
				if(diag_done)
				{
					pr_info("[B]%s(%d): total wait %d x 10ms ", __func__, __LINE__, (wait_retry-retry));
					break;
				}
			...
			{
				diag_payload_size = 5;
				*(((unsigned char *)diag_buf_copy)+4) = 0xFF;
			}

			if (copy_to_user((void *)ioarg, &diag_payload_size, 4))
			{
				pr_info("[B] %s(%d): ", __func__, __LINE__);
				return -EFAULT;
			}
	
			if (copy_to_user((void *)(ioarg + 4), diag_buf_copy, diag_payload_size))
			{
				pr_info("[B] %s(%d): ", __func__, __LINE__);
				return -EFAULT;
			}
...
#if 0
			printk(KERN_DEBUG "2. data is -->\n");
			for (i = 0; i < diag_payload_size; i++)
				printk(KERN_DEBUG "2.\t %x \t", *(((unsigned char *)diag_buf_copy)+i));
#endif	
		
			diagmem_free(driver, diag_buf_copy, POOL_TYPE_COPY);
			result = 1;
		}
		break;
#endif
...
/*[Arima_5830][bozhi_lin] implement diag send sync command for factory 20160223 begin*/
	...
	if (diag_buf_copy) 
	{
		if ( (*(((unsigned char *)diag_buf_copy)+0) == *(((unsigned char *)buf_copy)+0)) &&
				(*(((unsigned char *)diag_buf_copy)+1) == *(((unsigned char *)buf_copy)+1)) &&
				(*(((unsigned char *)diag_buf_copy)+2) == *(((unsigned char *)buf_copy)+2)) )
		{
			diag_payload_size = payload_size;
			err = copy_from_user(diag_buf_copy, buf + 4, diag_payload_size);
			if (err) {
				pr_info("[B] %s(%d): error copy_from_user diag_buf_copy", __func__, __LINE__);
				ret = -EFAULT;
				goto fail_free_copy;
