...
//[Camera][Kent][16062802][Begin] add interface to read and write dac
#include <linux/proc_fs.h>
#include <linux/seq_file.h>

#define LASER_FOCUS_ON_PATH "driver/vcm_value"
static struct proc_dir_entry *vcm_value;

static int vcm_dac_value=0;
struct msm_actuator_ctrl_t *vcm_ctrl;

static ssize_t vcm_value_write(struct file *file,const char __user *buffer,size_t count,loff_t *pos)
{
	char buf[] = "000000000";
	size_t len =  min((size_t)sizeof(buf) - 1, count);
	int input_value,rc;
	u16 addr, value;

	if(count > 0)
	{
	if (copy_from_user((void*)buf, buffer, len)) return count;
		buf[len] = 0;
		...
		pr_info("set_dac: 0x%x", input_value);
		addr=(u16)input_value >>4;
		value=(u16)(input_value &0x0f ) <<4;
		rc = vcm_ctrl->i2c_client.i2c_func_tbl->i2c_write(&vcm_ctrl->i2c_client,addr,value,MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0) {
			pr_err("%s: i2c write error:%d\n",
				__func__, rc);
				return count;
		}
		vcm_dac_value=input_value;
		
	}
	return count;
}

static int vcm_value_show(struct seq_file *m, void *v) {
  seq_printf(m, "%d\n",  vcm_dac_value);
  return 0;
}

static int vcm_value_open(struct inode *inode, struct  file *file) {
  return single_open(file, vcm_value_show, NULL);
}

static const struct file_operations vcm_value_fops = {
  .owner = THIS_MODULE,
  .open = vcm_value_open,
  .read = seq_read,
  .write  = vcm_value_write,
  .llseek = seq_lseek,
  .release = single_release,
};


//[Camera][Kent][16062802][end] add interface to read and write dac
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	//[Camera][Kent][16062803][Begin] add interface to read and write dac
	vcm_dac_value=
	//[Camera][Kent][16062803][End] add interface to read and write dac
...
static int32_t msm_actuator_platform_probe(struct platform_device *pdev)
{
	...
	msm_actuator_t->msm_sd.sd.devnode->fops =
		...
		//[Camera][Kent][16062801][Begin] add interface to read and write dac
	vcm_ctrl=msm_actuator_t;
	vcm_value = (struct proc_dir_entry*)proc_create(LASER_FOCUS_ON_PATH, 0666,NULL, &vcm_value_fops);
	if(NULL==vcm_value) 
		printk("%s,proc_create error.\n",__func__);
		//[Camera][Kent][16062801][end] add interface to read and write dac
