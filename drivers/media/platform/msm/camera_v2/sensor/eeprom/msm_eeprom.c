...
//[kent][16022606][begin] verify t4k37 product information
void liteon_t4k37_otp_format_calibration_data(struct msm_eeprom_ctrl_t *e_ctrl);
//[kent][16022606][begin] verify t4k37 product information
//[5830][Camera]verify ov5670 product information Person Liu 20160412 S
	...
	void __user *argp)
{
	...
	case CFG_EEPROM_GET_MM_INFO:
		...
		break;
		//[kent][16022601[begin] offer t4k37 product information
	case CFG_EEPROM_GET_PRODUCT_INFO:
		cdata->cfg.product_info.module_pn =
					e_ctrl->module_pn;
		cdata->cfg.product_info.module_sn =
							e_ctrl->module_sn;
		break;
		//[kent][16022601[end] offer t4k37 product information
	//[5833]Add for Main Camera 2nd source 20160614 S
	case CFG_EEPROM_GET_MODULE_ID:
		pr_err("%s CFG_EEPROM_GET_MODULE_ID\n", __func__);
		cdata->cfg.product_info.module_id =
					e_ctrl->module_id;
		break;
	//[5833]Add for Main Camera 2nd source 20160614 E
	default:
		break;
	}
	...
	void __user *argp)
{
	...
	case CFG_EEPROM_READ_CAL_DATA:
		...
		break;
		//[kent][16022604][begin] offer t4k37 product information
	case CFG_EEPROM_GET_PRODUCT_INFO:
		cdata->cfg.product_info.module_pn =
					e_ctrl->module_pn;
		cdata->cfg.product_info.module_sn =
					e_ctrl->module_sn;
		break;
		//[kent][16022604][end] offer t4k37 product information
	//[5833]Add for Main Camera 2nd source 20160614 S
	case CFG_EEPROM_GET_MODULE_ID:
		pr_err("%s CFG_EEPROM_GET_MODULE_ID\n", __func__);
		cdata->cfg.product_info.module_id =
...
//[5833]Add for Main Camera 2nd source 20160614 S
	...
	if(strstr(e_ctrl->eboard_info->eeprom_name,"ov8858_otp")){
		if((e_ctrl->cal_data.mapdata[0]&0xc0)==0x40)	
			e_ctrl->module_id = e_ctrl->cal_data.mapdata[1];
		else
			e_ctrl->module_id = e_ctrl->cal_data.mapdata[9];
		pr_err("%s Person Test flag = 0x%x, module id = 0X%x\n", __func__,e_ctrl->cal_data.mapdata[0],e_ctrl->module_id);

	}
#else
//[kent][16022605][begin] verify t4k37 product information
	if(strstr(e_ctrl->eboard_info->eeprom_name,"t4k37_otp"))
		liteon_t4k37_otp_format_calibration_data(e_ctrl);
//[kent][16022605][end] verify t4k37 product information
//[5830][Camera]verify ov5670 product information Person Liu 20160412 S
	if(strstr(e_ctrl->eboard_info->eeprom_name,"ov5670_otp"))
		liteon_ov5670_otp_format_calibration_data(e_ctrl);
//[5830][Camera]verify ov5670 product information Person Liu 20160412 E
#endif
//[5833]Add for Main Camera 2nd source 20160614 E
