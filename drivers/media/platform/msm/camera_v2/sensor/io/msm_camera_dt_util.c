...
{
	int rc = 0, i = 0;
	//[camera][Kent][16021601][begin]fix when the power of eeprom not use vreg power on dtsi
	#if 0
	uint32_t count = 0;
	#else
		int32_t count = 0;
	#endif
    //[camera][Kent][16021601][end]fix when the power of eeprom not use vreg power on dtsi
	...
	//[camera][Kent][16021602][begin]fix when the power of eeprom not use vreg power on dtsi
#if 0
	if (!count)
		return 0;
#else
	if (count <= 0)
	{
		pr_err("%s read qcom,cam-vreg-name failed \n", __func__);
		return 0;
	}
#endif
	//[camera][Kent][16021602][end]fix when the power of eeprom not use vreg power on dtsi
