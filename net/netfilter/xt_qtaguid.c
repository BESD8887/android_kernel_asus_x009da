...
static int qtaguid_ctrl_proc_show(struct seq_file *m, void *v)
{
	...
	if (sock_tag_entry != SEQ_START_TOKEN) {
		...
		//Eric modify for for CTS testAppFailAccessPrivateData fail
			 ...
			 int cnt_set)
{
	...
	//EricLee 2016-09-14 modify for CTS testAppFailAccessPrivateData fail start
	//if (get_atag_from_tag(tag) && !can_read_other_uid_stats(stat_uid)) {
	  if (!can_read_other_uid_stats(stat_uid)) {
	//EricLee 2016-09-14 modify for CTS testAppFailAccessPrivateData fail end
